...
###################### DELETE ####################################		
	...
	if(isset($_POST['delete_all']) && $_POST['employer_id'] != "" && sizeof($_POST['employer_id']) != 0 ){
		//$employer->admin_comments = $_POST['reject_reason'];
...
/////////pending employer
if(isset($_POST['approved_all']) && $_POST['employer_id'] != "" && sizeof($_POST['employer_id']) != 0 ){
	foreach( $_POST['employer_id'] as $key => $value ){
		if($value != "" ) {
			$employer->id = $value;
			if( $employer->approved_account() ){ $success=true;}
		}
	}
	if($success){
		$session->message("<div class='success'>Employer(s) has been approved </div>");
		redirect_to( $_SERVER['PHP_SELF'] );
		die;
	}
}
...
###############################################################################
		...
		$sql = "SELECT * FROM ".TBL_EMPLOYER. " WHERE employer_status='Pending' ";
		$total_count = sizeof(Employer::find_by_sql( $sql ) );
		unset($sql);
		...
		$sql.=" WHERE employer_status='Pending' ";
		$sql.=" LIMIT {$per_page} ";
		$sql.=" OFFSET {$offset} ";
...
$smarty->assign('rendered_page', $smarty->fetch('admin/employer_pending.tpl') );
