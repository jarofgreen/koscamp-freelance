...
if( isset($_GET['id']) && $_GET['action'] == "active")
{
	$employee->id = $_GET['id'];
	if(	$employee->approved_account() ){
		$session->message("<div class='success'>Employee has been approved</div>");
		redirect_to( $_SERVER['PHP_SELF'] );
		die;
	}
}
...
###############################################################################
		...
		$total_count = Employee::count_all_pending();
...
$smarty->assign('rendered_page', $smarty->fetch('admin/manage_employee_pending.tpl') );
