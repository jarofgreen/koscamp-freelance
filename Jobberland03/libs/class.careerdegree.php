...
class CareerDegree extends DatabaseObject{
	protected static $table_name = TBL_CAREER_DEGREE;
	protected static $db_fields = array('id', 'var_name', 'career_name',  'is_active');
		...
		if( $result ) {
			while ( $c = $db->fetch_object( $result ) ) {
				if( empty( $c->var_name) ||  $c->var_name == ''){
					$var_name = $this->mod_write_check( $c->career_name, NULL );
	...
	public static function find_by_career( $name=null){
		global $database, $db;
		$sql = " SELECT career_name FROM ". self::$table_name;
		$sql .= " WHERE career_name='".$db->escape_value($name)."'";
		$sql .= " LIMIT 1 ";
		$result = $database->query( $sql );
		$row = $db->fetch_array($result);
		return !empty($row) ? $row['career_name'] : false;
	}
...
############################# Saving and Updating ##########################	
	public function save() {
		...
		if( empty($this->career_name) ){
			$this->errors[]= get_lang('errormsg', 20);
		}
