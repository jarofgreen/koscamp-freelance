...
class Category extends DatabaseObject{
	...
	protected static $db_fields = array('id', 'var_name', 'cat_name');
		...
		if( $database->num_rows( $result ) > 0 ) {
			while ( $c = $db->fetch_object( $result ) ) {
				if( empty( $c->var_name) ||  $c->var_name == ''){
					$var_name = $this->mod_write_check( $c->cat_name, NULL );
	...
	public static function find_by_category( $name=null){
		global $database, $db;
		$sql = " SELECT cat_name FROM ". self::$table_name;
		$sql .= " WHERE cat_name='".$db->escape_value($name)."'";
		$sql .= " LIMIT 1 ";
		$result = $database->query( $sql );
		$row = $db->fetch_array($result);
		return !empty($row) ? $row['cat_name'] : false;
	}	
	...
	public function save() {
		...
		if( empty($this->cat_name) ){
			$this->errors[]= get_lang('errormsg', 19);
		}
