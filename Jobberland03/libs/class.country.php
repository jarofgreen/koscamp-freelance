...
class Country extends DatabaseObject{
	protected static $table_name = TBL_COUNTRY;
	protected static $db_fields = array('id', 'loc', 'code', 'var_name', 'name', 'enabled');
		...
		if( $result ) {
			while ( $c = $db->fetch_object( $result ) ) {
				if( empty( $c->var_name) ||  $c->var_name == ''){
					...
					$sql .= " SET var_name='".$this->mod_write_check( $c->name)."' ";
					$sql .= " WHERE id=".$c->id;
					$database->query( $sql );
				}
			}
	...
	public static function find_by_code( $country_code="GB" ){
		global $database, $db;
		$sql = " SELECT * FROM ". self::$table_name;
		$sql .= " WHERE code= '".$db->escape_value($country_code)."' ";
		$sql .= " LIMIT 1 ";
 	...
 	public static function find_by_loc_var( $country="GB" ){
		global $database, $db;
		$sql = " SELECT * FROM ". self::$table_name;
		$sql .= " WHERE var_name= '".$db->escape_value($country)."' ";
		$sql .= " LIMIT 1 ";
