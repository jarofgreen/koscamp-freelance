...
class Education extends DatabaseObject{
	protected static $table_name = TBL_EDUCATION;
	protected static $db_fields = array('id', 'var_name', 'education_name',  'is_active');
		...
		if( $result ) {
			while ( $c = $db->fetch_object( $result ) ) {
				if( empty( $c->var_name) ||  $c->var_name == ''){
					$var_name = $this->mod_write_check( $c->education_name, NULL );
...
############################# Saving and Updating ##########################	
	public function save() {
		...
		if( empty($this->education_name) ){
			$this->errors[]=get_lang('errormsg', 13);
		}
