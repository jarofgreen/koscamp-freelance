...
<p><strong>{$employer_fullname}</strong></p>
    ...
    <td><input type="hidden" name="txt_country" id="txt_country" value="{$countrycode}" />
        ...
        <div style="float:left; min-width:178px; padding-right:10px;"><label>State: </label>
            <div id="stateprovince_auto">
                {if $lang.states|@count > 0}
                    ...
                    {html_options options=$lang.states selected=$smarty.session.add_job.state}
                    </select>
                { else }
                    <input class="text_field required" name="txtstateprovince" type="text" size="30" maxlength="100" value="{$smarty.session.add_job.state}" />
               { /if}                
            </div>
        </div>
        ...
        <div id="county_auto">
          { if $lang.counties|@count > 0}
            ...
            {html_options options=$lang.counties selected=$smarty.session.add_job.county}
            </select>
          { else }
            <input name="txtcounty" type="text" size="30" maxlength="100" value="{$smarty.session.add_job.county}" />
          { /if}
            ...
            <div id="city_auto">
              { if $lang.cities|@count > 0}
                <select class="select" name="txtcity" >
                  {html_options options=$lang.cities selected=$smarty.session.add_job.city}
                </select>
              { else }
                <input name="txtcity" type="text" size="30" maxlength="100" value="{$smarty.session.add_job.city}" />
              { /if}
